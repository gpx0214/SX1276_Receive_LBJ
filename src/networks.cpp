...
bool connectWiFi(const char *ssid, const char *password, int max_tries, int pause) {
    ...
    // WiFi.disconnect(true); // add
    WiFiClass::mode(WIFI_STA);
    // WiFi.setScanMethod(WIFI_ALL_CHANNEL_SCAN); // add
    WiFi.begin(ssid, password);
    do {
        delay(pause);
        Serial.print(".");
    } while (!isConnected() && retry++ < max_tries);
    if (isConnected())
        Serial.print("SUCCESS.");
    else
        Serial.printf("FAILED %d.", WiFiClass::status());
...
static const char *reset_reason_str[] = {
    ...
    "EXT reset_pin",  // "ESP_RST_EXT, Reset by external pin (not applicable for ESP32)",
    "SW esp_restart", // "ESP_RST_SW, Software reset via esp_restart",
...
void onTelnetConnect(String ip) {
    ...
    telnet.printf("%d start %llu.%03llus\n\r%s\n\r", start_cnt, millis64()/1000, millis64()%1000, printResetReason(esp_reset_reason()));
