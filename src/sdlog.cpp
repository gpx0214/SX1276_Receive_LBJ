...
void SD_LOG::getFilename(const char *path) {
    ...
    if (timein.tm_year + 1900 > 2001) {
        sprintf(filename, "%02d%02d%02d%02d.txt", (timein.tm_year + 1900) % 100, timein.tm_mon + 1,timein.tm_mday, timein.tm_hour/6*6);
...
void SD_LOG::getFilenameCSV(const char *path) {
    ...
    getLocalTime(&timein, 1);
    if (timein.tm_year + 1900 > 2001) {
        sprintf(filename_csv, "%02d%02d%02d%02d.csv", (timein.tm_year + 1900) % 100, timein.tm_mon + 1, timein.tm_mday, timein.tm_hour/6*6);
        Serial.printf("[SDLOG] using %s \n", filename_csv);
        sd_csv = true;
        return;
    }
...
void SD_LOG::writeHeaderCSV() { // TODO: needs more confirmation about title.
    ...
    // }
    if (csv.size() < 100) {
        // gbk "时间,方向,级别,车次,速度,公里标,机车编号,线路,经度,纬度,RSSI,FER,PPM(CURRENT),原始数据,错误\n"
        csv.printf("\xca\xb1\xbc\xe4,\xb7\xbd\xcf\xf2,\xbc\xb6\xb1\xf0,\xb3\xb5\xb4\xce,\xcb\xd9\xb6\xc8,\xb9\xab\xc0\xef\xb1\xea,\xbb\xfa\xb3\xb5\xb1\xe0\xba\xc5,\xcf\xdf\xc2\xb7,\xbe\xad\xb6\xc8,\xce\xb3\xb6\xc8,RSSI,FER,PPM(CURRENT),\xd4\xad\xca\xbc\xca\xfd\xbe\xdd,\xb4\xed\xce\xf3\n");
        if (sd_log) {
