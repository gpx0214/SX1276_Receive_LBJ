...
void setup() {
    ...
    if (u8g2) {
        ...
        u8g2->drawUTF8(80, 8, printResetReason(reset_reason));
    ...
    if (have_sd) {
        ...
        sd1.append("电池电压 %1.2fV 重启次数 %d 复位原因0x%02x %s\n", battery.readVoltage() * 2, start_cnt, reset_reason, printResetReason(reset_reason));
        sd1.append(2, "调试等级 %d\n", LOG_VERBOSITY);
#ifdef HAS_RTC
        if (have_rtc) {
...
void handleKey(int8_t key) {
    if (key == KEY_OK) return handleKey5(key);
    btn_pressed = true;
    oled.updateSleepTimestamp();
    // Serial.printf("[D] GPIO 34: %d ADU, KEY %d\n", btn_level, 1);
...
/*
            if (btn_level >= 580 && btn_level <= 660 && abs(btn_level - btn_level_prev) < 10) {
                ...
                handleKey5(KEY_OK);
*/
            if (btn_level >= 1820 && btn_level <= 1920 && abs(btn_level - btn_level_prev) < 10) {
                handleKey(KEY_DOWN);
            } else if (btn_level >= 1130 && btn_level <= 1230 && abs(btn_level - btn_level_prev) < 10) { // 1460-1490
                handleKey(KEY_UP);
            } else if (btn_level >= 2470 && btn_level <= 2570) { // 2250-2280
                handleKey(KEY_RIGHT);
            } else if (btn_level >= 3250 && btn_level <= 3350) { // 2990-3110
                handleKey(KEY_LEFT);
            } else if (btn_level >= 4090 && btn_level <= 4096) {
                handleKey5(KEY_OK);
