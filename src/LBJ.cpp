...
size_t bcdchar(char* dst, char* in, size_t start, size_t len) {
    ...
    for(size_t i=start; i+2<=start+len; i+=2) {
        ...
        if ((0x00 < c && c < 0x20) || c == 0x2c || (0x7e < c && c < 0xA1) || c > 0xfe) {
            c = '?';
        }
        dst[idx++]=c;
    }
...
static const char* html_color[] = {"#ffffff", "#00ffff", "#eeee00", "#ccccff", "#00ff00", "#cc00cc", "#ff00ff", "#ff0000", "#ff0000"};
...
size_t color_html_sprintf(int8_t color, bool under, char* dst, const char* format, ...) {
    size_t idx=0;
    if (color) {
        idx += sprintf(dst+idx, "<font color='%s'>", html_color[color%8]); // , under?";4":""); 
        // p.ebit[word_idx]-even_parity(p.correct_bin[word_idx])
        // even_parity(p.correct_bin[word_idx])
    }
    va_list args;
...
size_t LBJ_html(char* dst, const PagerClient::poc32 &p, const struct lbj_data &l, const struct rx_info &r) {
    int32_t idx = 0;
    ...
    // idx += sprintf(dst+idx, "[LBJ]");
    idx += sprintf(dst+idx, "%6.1f ", r.rssi);
    idx += sprintf(dst+idx, r.date_time_str);
    idx += sprintf(dst+idx, ":");
    if (l.type == 8) {
        idx += color_html_sprintf(p.Ebit(1,1), false, dst+idx, "时间%c%s", p.text[5], l.train);
        idx += sprintf(dst+idx, "\n");
        return idx;
    }

    idx += color_html_sprintf(p.Ebit(5,5), false, dst+idx, "%-2s", l.lbj_class);
    idx += color_html_sprintf(p.Ebit(1,1), false, dst+idx, "%5s", l.train);
    idx += color_html_sprintf(p.Ebit(2,2), false, dst+idx, " %3skm/h", l.speed);
    idx += color_html_sprintf(p.Ebit(3,3), false, dst+idx, " %skm", l.position);
    idx += color_html_sprintf(p.Ebit(0,0), false, dst+idx, "%2s", function_code_map[l.direction]);
    ...
    idx += color_html_sprintf(p.Ebit(5,7), false, dst+idx, " %s%s", l.loco, l.loco_end);
    idx += sprintf(dst+idx, "[");
    idx += sprintf(dst+idx, "<font color='%s'>", html_color[p.Ebit(8,10)]); //, false?";4":"");
    idx += gbk2utf8(dst+idx, l.route, 8);
    idx += sprintf(dst+idx, "</font>");
    idx += sprintf(dst+idx, "]");
    idx += color_html_sprintf(p.Ebit(11,12), false, dst+idx, " %s", l.lon);
    idx += color_html_sprintf(p.Ebit(13,14), false, dst+idx, " %s", l.lat);
    idx += sprintf(dst+idx, "\n");
    dst[idx] = '\0';
    return idx;
}
...
size_t formatCSV(char* buffer, const PagerClient::poc32 &p, const lbj_data &l, const rx_info &rx, uint32_t ids) {
    ...
    if (buffer[idx-1] != 'A' && buffer[idx-1] != 'B') {
        idx--;
    }
...
size_t formatCSVfile(char* buffer, const PagerClient::poc32 &p, const lbj_data &l, const rx_info &rx) {
    ...
    idx += sprintf(buffer+idx, ",%s,%s,%s,%s,%s", function_code_map_gbk[l.direction], l.lbj_class, l.train, l.speed, l.position);
